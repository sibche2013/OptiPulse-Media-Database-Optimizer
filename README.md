...
* **🛒 مدیریت Action Scheduler ووکامرس:** پاک‌سازی فوری لاگ‌های تاریخ‌گذشته ووکامرس و قابلیت تنظیم دلخواه دوره نگهداری خودکار (مثلاً ۲ یا ۵ روز به جای ۳۰ روز پیش‌فرض).
...
این افزونه به‌طور کامل شیءگرا و با رعایت استانداردهای رسمی مخزن وردپرس (WordPress.org) توسعه یافته است.

### ۱. معماری شیءگرا و Namespace
افزونه از الگوی طراحی **Singleton** بهره می‌برد و تمامی کدها در `namespace OptiPulseMediaDatabaseOptimizer` ایزوله شده‌اند تا از هرگونه تداخل (Conflict) با سایر افزونه‌ها یا قالب‌ها جلوگیری شود.
...
تمامی عملیات‌های حذف و جایگزینی فایل‌ها بدون استفاده از توابع ناامن PHP و کاملاً از طریق توابع بومی وردپرس (`wp_delete_file`) انجام می‌شود.

### ۴. سفارشی‌سازی Fancybox
کدهای مربوط به اجرای Fancybox از طریق ساختار `NOWDOC` در PHP تزریق می‌شوند. برای تغییر پارامترهای انیمیشن، دکمه‌ها و... می‌توانید متد `enqueue_frontend_assets` را در کلاس `Plugin` شخصی‌سازی کنید.

---

### 📸 پیش‌نمایش پنل مدیریت

![OptiPulse Preview](sc.png)

---

## 🤝 راهنمای مشارکت (Contribution)

مراحل مشارکت در توسعه:
...
1. پوشه افزونه را با نام `optipulse-media-database-optimizer` در مسیر `wp-content/plugins/` قرار دهید.
2. افزونه **OptiPulse: Media & Database Optimizer** را از بخش «افزونه‌ها» در پیشخوان وردپرس فعال کنید.
3. از منوی کناری پیشخوان به مسیر **بهینه‌ساز هوشمند** بروید و پیکربندی را انجام دهید.
...
## 📄 لایسنس
این پروژه تحت لایسنس **GPLv2 or later** (استاندارد رسمی وردپرس) منتشر شده است.